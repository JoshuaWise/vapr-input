...
module.exports = ({ deferred = false, anyCharset = false, default: defaultParser, ...parsers } = {}) => {
	...
	if (defaultParser != null) {
		if (typeof defaultParser === 'string') {
			if (!parsers.has(defaultParser)) throw new TypeError(`Default parser "${defaultParser}" is not defined`);
			defaultParser = parsers.get(defaultParser);
		} else if (typeof defaultParser !== 'function') {
			throw new TypeError('Expected defaultParser to be a string or function');
		}
	}
